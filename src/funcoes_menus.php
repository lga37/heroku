...
function menuInfinitoBS4($rows, $hasChildren, $parentId, $param='place_id',$valor=false,$level = 0) {
    ...
    $link="$url?$param=$valor";

    if($parentId==0){
      $menu_html = PHP_EOL.'<ul class="list-group">';
    }else{
      if (isset($hasChildren[$parentId])) {
        $menu_html = PHP_EOL.'<ul class="collapse" id="submenu-'.$parentId.'">';
      }
    }
    ...
    foreach ($rows as $id => $row) {
        if ($row["pai"] == $parentId) {
            if (isset($hasChildren[$id])) {
                $menu_html .= '<li><a class="list-group-item" data-toggle="collapse" id="btn-'.$row['id'].'" data-target="#submenu-'.$row['id'].'" href="#">'.$row["nome"].'+</a>';
                $menu_html .= menuInfinitoBS4($rows,$hasChildren,$id,$param,$valor,$level + 1);
                $menu_html .= '</li>';
            }else{
                $id=$row['id'];
                $nome=$row['nome'];
                $menu_html .= "<li><a class='list-group-item' href='$link&categ_id=$id'>$nome</a></li>";
            }
        }
    }
